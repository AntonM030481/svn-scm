...
suite("Encoding detection", () => {
  ...
  test("detects BOM encodings before statistical detection", () => {
    assert.strictEqual(detectEncoding(Buffer.from([0xfe, 0xff, 0, 65])), "utf16be");
    assert.strictEqual(detectEncoding(Buffer.from([0xff, 0xfe, 65, 0])), "utf16le");
    assert.strictEqual(detectEncoding(Buffer.from([0xef, 0xbb, 0xbf, 65])), "utf8");
  });
  ...
  test("limits statistical detection to the first 64 KiB", () => {
    ...
    assert.strictEqual(detectEncoding(Buffer.concat([asciiPrefix, CP1251])), null);
  });
});
