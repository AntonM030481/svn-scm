...
suite("Staging Directory Status Tests", () => {
  test("hidden descendant cleanup scopes svn status to the staged directory", async () => {
        ...
        async exec(args: string[]) {
          ...
          return {
            stdout: [
              ...
              `    <entry path="${path.join(relativeDirectory, "hidden.txt")}">`,
    ...
    try {
      ...
      }).cleanupHiddenDirectoryAdditions(repository, directory, []);
    } finally {
      coordinator.dispose();
    }
